...
def test_hendrycks_math(model, data_path, start=0, end=MAX_INT, batch_size=1, tensor_parallel_size=1):
    ...
    llm = LLM(model=model,tensor_parallel_size=tensor_parallel_size, enforce_eager=True, dtype = "float16", gpu_memory_utilization=0.8)
...
def parse_args():
    ...
    parser.add_argument("--model", type=str, default='iterative_rft/model1')  # model path
